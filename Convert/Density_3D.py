...
class LM_density:
    ...
    def readCatalog(self, filename, colD='distK50_RJCE', colDlow='distK85_RJCE', colDup='distK15_RJCE', unitD='pc', \
                    colKmag='Kmag_2mass', colJmag='Jmag_2mass', colPid='plateserial', colGlon='glon', colGlat='glat',
                    colFeh='feh'):
        '''
        It can only read fits table with flexable columns.
...
def save_file(dr3, ind, D, Dlow, Dup, Z, R, r, lnnu, filename):
    ...
    '''
    a = np.array([dr3.obsid[ind].reshape((len(D))), dr3.ra[ind].reshape((len(D))), dr3.dec[ind].reshape((len(D))), \
                  dr3.glon[ind].reshape((len(D))), dr3.glat[ind].reshape((len(D))), dr3.teff[ind].reshape((len(D))), \
                  dr3.logg[ind].reshape((len(D))), dr3.rv[ind].reshape((len(D))), \
                  dr3.EW_Mgb[ind].reshape(len(D)), dr3.M_K50[ind].reshape((len(D))), \
                  dr3.M_K50[ind].reshape((len(D))) - dr3.M_K15[ind].reshape((len(D))), \
                  dr3.M_K85[ind].reshape((len(D))) - dr3.M_K50[ind].reshape((len(D))), \
                  dr3.AK_RJCE[ind].reshape((len(D))), dr3.Kmag_2mass[ind].reshape((len(D))), \
                  D, Dlow, Dup, Z, R, r, lnnu]).T
    # print a,np.shape(a)
    np.savetxt(filename, a, \
               fmt='%d %.5f %+.5f %.5f %+.5f %.0f %.2f %.0f %.5f %.2f %.2f %.2f %.3f %.3f %.2f %.2f %.2f %.2f %.2f %.2f %.2f', \
               delimiter='', \
               header='obsid ra dec l b teff logg rv Mgb MK MKerr_low MKerr_up AK K dist disterr_low disterr_up Z R r_gc lnnu')